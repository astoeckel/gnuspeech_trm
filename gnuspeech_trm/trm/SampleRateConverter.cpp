...
SampleRateConverter::SampleRateConverter(int sampleRate, float outputRate,
 ...
 *
 ******************************************************************************/
void SampleRateConverter::initializeConversion(int sampleRate, float outputRate)
{
	...
	if (sampleRateRatio_ >= 1.0) {
		...
		phaseIncrement_ =
		    (unsigned int)rint(sampleRateRatio_ * (double)FRACTION_RANGE);
	}
 ...
 *
 ******************************************************************************/
double SampleRateConverter::Izero(double x)
{
 ...
 *
 ******************************************************************************/
void SampleRateConverter::initializeBuffer()
{
 ...
 *
 ******************************************************************************/
void SampleRateConverter::dataFill(double data)
{
 ...
 *
 ******************************************************************************/
void SampleRateConverter::dataEmpty()
{
	...
	if (sampleRateRatio_ >= 1.0) {
		while (emptyPtr_ < endPtr) {
			...
			double interpolation =
			    (double)mValue(timeRegister_) / (double)M_RANGE;

			/*  COMPUTE THE LEFT SIDE OF THE FILTER CONVOLUTION  */
			int index = emptyPtr_;
			for (int filterIndex = lValue(timeRegister_);
			     filterIndex < FILTER_LENGTH; srDecrement(&index, BUFFER_SIZE),
			         filterIndex += filterIncrement_) {
				output +=
				    (buffer_[index] * (h_[filterIndex] +
				                       (deltaH_[filterIndex] * interpolation)));
			}
			...
			for (int filterIndex = lValue(timeRegister_);
			     filterIndex < FILTER_LENGTH; srIncrement(&index, BUFFER_SIZE),
			         filterIndex += filterIncrement_) {
				output +=
				    (buffer_[index] * (h_[filterIndex] +
				                       (deltaH_[filterIndex] * interpolation)));
 ...
 *
 ******************************************************************************/
void SampleRateConverter::srIncrement(int *pointer, int modulus)
{
	if (++(*pointer) >= modulus) {
		(*pointer) -= modulus;
	}
 ...
 *
 ******************************************************************************/
void SampleRateConverter::srDecrement(int *pointer, int modulus)
{
	if (--(*pointer) < 0) {
		(*pointer) += modulus;
	}
 ...
 *
 ******************************************************************************/
void SampleRateConverter::flushBuffer()
{
